...
export const sortUpcomingEventByDate = (events) => {
  ...
  const _events = events.filter((event) => !isAfter(date, new Date(event.eventDate)))
...
export const sortPastEventsByDate = (events) => {
  ...
  const prevAndCurrentYear = _events.filter((event) => {
    ...
    return isThisYear(new Date(event.eventDate)) || Number(getDateBreakdown(event.eventDate).year) >= lastYear
  })
...
export const getDateBreakdown = (_date) => {
  ...
  return { month, longMonth, numericMonth, day, year, hour, hour24, minute, hoursAndMinutes }
}
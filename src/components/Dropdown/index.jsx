    ...
    <section className="w-11/12 mx-auto cursor-pointer relative">
      ...
      {showDropdown ?
        <ul className="absolute bg-white border border-black border-opacity-20 left-[auto] right-0 top-10 min-w-[160px] rounded-md shadow-[0_6px_12px_0_rgba(0,0,0,0.18)]">
          {showAlt ? <li className=" px-5 border-b py-2">Hi, {userName}</li> : <li className="px-5 hover:bg-[#f5f5f5] text-[#262626] py-2"><Link to={paths.jobs} className="block">Jobs</Link></li>}

          {showEditProfile ? <li className="px-5 hover:bg-[#f5f5f5] text-[#262626] py-2"><Link to={`/edit/jobs/${userName}`} className="block">Edit Profile</Link></li> : <li className="px-5 hover:bg-[#f5f5f5] text-[#262626] py-2"><Link to={`/jobs/${userName}`} className="block">Profile</Link></li>}

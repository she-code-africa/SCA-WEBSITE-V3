    ...
    <header className="flex justify-between items-center py-5 px-8">
      <Link to={paths.home}>
        <img
          src={logo}
          alt="SCA Logo"
          className="object-contain __shecodelogo min-w-[48px]"
        />
      </Link>
      <ul className="md:flex justify-between hidden lg:text-[12px] md:text-[10px] xl:text-[15px] text-primary-dark-pink py-[10px] xl:px-[20px] md:px-[5px] fixed ">
        {menus.map((menu) => (
          <li
            key={menu.name}
            className={`lg:pr-12 md:pr-5 __shecodelink  ${
          ...
          >
            {menu.submenu ? (
              <div className="relative">
                <div
                  className={`font-bold px-3 transition-all duration-[400s] tracking-[0.36px] flex items-center`}
                ...
                {subMenuOpen ? (
                  <ul className="pt-4 absolute top-9 -left-5 -right-5 border border-gray-300 bg-white shadow-md">
                    {menu.submenu.map((child) => (
                      ...
                      >
                        <Link
                          className={`no-underline font-bold px-3 block transition-all duration-[400s] tracking-[0.36px] ${
                            page === child.name
                              ? "border-b border-primary-main-pink transform scale-105"
              ...
              <Link
                className={`no-underline font-bold block transition-all duration-[400s] tracking-[0.36px] ${
                  page === menu.name
            ...
            {menus.map((menu) => (
              ...
              >
                {menu.submenu ? (
                  <>
                    <div
                      className={`font-bold px-3 transition-all duration-[400s] tracking-[0.36px] border-b border-gray-400 border-opacity-20 flex items-center`}
                      onClick={() => setSubMenuOpen(!subMenuOpen)}
                    >
                      <p className="pr-3">{menu.text}</p>
                      <Down />

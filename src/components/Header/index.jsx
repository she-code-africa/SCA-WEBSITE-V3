  ...
  return (
    <header className="sticky top-0 w-full md:bg-white/80 bg-white z-[1] md:py-8 py-4">
      <div className="justify-between gap-8 items-center lg:w-10/12 mx-auto hidden md:flex px-5 lg:px-0">
                ...
                <ul className="absolute bg-white top-12 pt-5 px-5 w-max">
                  {menu.list.map((list, index) => (
                    <li
                      ...
                      {list?.external ? (
                        <a href={list.to} target="_blank" rel="noreferrer">
                          {list.text}
                        </a>
                      ) : (
        ...
        <Link
          to={paths.donate}
          className="bg-primary-main-pink rounded-lg py-5 px-12 text-white hover:text-white">
          Donate
        </Link>
      </div>
      <div className="md:hidden flex justify-between place-items-center px-8">
        ...
        <div
          className="flex flex-col justify-between items-center cursor-pointer w-[30px] h-[8px] transition-transform duration-300 ease-in-out"
          onClick={handleClick}>
          <div
            className={`line bg-[#1A1A1A] h-[2px] w-full block transition-transform duration-300 ease-in-out translate-y-0 ${isOpen ? 'translate-y-[2.5px] rotate-45' : ''
              }`}></div>
          <div
          ...
          <div
            className={`line bg-[#1A1A1A] h-[2px] w-full block transition-transform duration-300 ease-in-out translate-y-[12px] ${isOpen ? 'translate-y-[-2.5px] -rotate-45' : ''
              }`}></div>
        </div>
              ...
              {menus.map((menu, index) => (
                <li
                  ...
                  {menu?.list ? (
                    ...
                    <Link to={menu.to} className={`block font-medium mb-6`}>
                      {menu.text}
                    </Link>
                  )}
                  {menu?.list && (

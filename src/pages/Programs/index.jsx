          ...
          <div className="md:w-9/12 mx-auto">
            <ul className="flex flex-wrap border-b border-[#ddd] overflow-x-auto px-10 justify-center">
              {programsCategories.map((category) => (
                <li className={`text-center font-semibold mr-8 text-base py-2 tracking-wider transition-all duration-700 ${activeTab === category?._id ? 'text-primary-dark-pink border-b-[3px] border-primary-dark-pink' : 'text-[#0D2543]'}`} role={'button'} key={category._id} onClick={() => setActiveTab(category?._id)}>{category?.name}</li>
              ))}
            </ul>
          </div>
          {programsCategories.map((category) => (
          activeTab === category._id && <section className="animate__animated animate__fadeIn animate__slow">

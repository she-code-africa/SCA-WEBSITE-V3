          ...
          <div>
            {communityFaq.map(({ heading, content }, index) => (
              <AccordionComponent
                ...
                heading={heading}
                id={index}
                active={communityFaqS}
                setIsActive={setCommunityFaq}
              >
                {content}
              </AccordionComponent>
            ))}
          ...
          <div>
            {partnershipFaq.map(({ heading, content }, index) => (
              <AccordionComponent
                ...
                heading={heading}
                id={index}
                active={partnershipFaqS}
                setIsActive={setPartnershipFaq}
              >
                {content}
              </AccordionComponent>
            ))}
          ...
          <div>
            {membershipFaq.map(({ heading, content }, index) => (
              <AccordionComponent
                ...
                heading={heading}
                id={index}
                active={membershipFaqS}
                setIsActive={setMembershipFaqS}
              >
                {content}
              </AccordionComponent>
            ))}
...
const AccordionComponent = ({ heading, children, active, setIsActive, id }) => {
  return (
    <article className="rounded shadow-[0_1px_5px_rgba(0,0,0,5%)] my-3">
      <div
        className=" cursor-pointer bg-primary-lighter-pink text-primary-dark-pink py-4 px-5 rounded-t-[3px] border-[#ddd] border-b text-base font-medium flex justify-between items-center"
        onClick={() => (active === id ? setIsActive(null) : setIsActive(id))}
      >
        <h4 className="panel-title">{heading}</h4>
        <FontAwesomeIcon icon={active === id ? faMinus : faPlus} size="1x" />
      </div>
      {active === id ? (
        <div className="p-4 border-t border-[#ddd]">{children}</div>
      ) : (
        ""
      )}
    </article>
